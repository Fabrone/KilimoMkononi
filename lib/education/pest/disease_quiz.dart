...
class _DiseaseQuizScreenState extends State<DiseaseQuizScreen> {
    ...
    try {
      ...
      if (mounted) {
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (_) => DiseaseQuizPlayScreen(
              payload: fullPayload,
  ...
  void _showQuizBuilder() {
    showDialog(
      context: context,
      builder: (_) => DiseaseQuizBuilderDialog(
        classId: widget.classId,
  ...
  Widget build(BuildContext context) {
    ...
    // No AppBar - parent (disease_home) provides title and back button
    return SafeArea(
        child: LayoutBuilder(
...
// ==================== QUIZ PLAY SCREEN ====================
class DiseaseQuizPlayScreen extends StatefulWidget {
  final Map<String, dynamic> payload;
  final String classId;

  const DiseaseQuizPlayScreen({
    super.key,
  ...
  @override
  State<DiseaseQuizPlayScreen> createState() => _DiseaseQuizPlayScreenState();
}

class _DiseaseQuizPlayScreenState extends State<DiseaseQuizPlayScreen> {
  int _idx = 0;
  int _score = 0;
...
// ==================== QUIZ BUILDER DIALOG ====================
class DiseaseQuizBuilderDialog extends StatefulWidget {
  final String classId;
  final String contentType;

  const DiseaseQuizBuilderDialog({
    super.key,
    required this.classId,
  ...
  @override
  State<DiseaseQuizBuilderDialog> createState() => _DiseaseQuizBuilderDialogState();
}

class _DiseaseQuizBuilderDialogState extends State<DiseaseQuizBuilderDialog> {
  ...
  Future<void> _saveQuiz() async {
    ...
    await FirestoreHelper.addContentToClass(
      ...
      {
        'type': 'quiz',
        'title': _titleCtrl.text.trim().isEmpty ? 'Disease Quiz' : _titleCtrl.text.trim(),
  ...
  Widget build(BuildContext context) => AlertDialog(
        title: const Text('Create Disease Quiz'),
        content: SizedBox(
