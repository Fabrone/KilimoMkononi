...
class DiseaseSimulationScreen extends StatefulWidget {
  final EduRole role;
  final String schoolName;
  final String classId;

  const DiseaseSimulationScreen({
    super.key,
    required this.role,
  ...
  @override
  State<DiseaseSimulationScreen> createState() => _DiseaseSimulationScreenState();
}

class _DiseaseSimulationScreenState extends State<DiseaseSimulationScreen> {
    ...
    try {
      ...
      if (mounted) {
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (_) => DiseaseSimulationPlayScreen(
              payload: fullPayload,
  ...
  void _showSimBuilder() {
    showDialog(
      context: context,
      builder: (_) => DiseaseSimulationBuilderDialog(
        classId: widget.classId,
                ...
                builder: (context, snapshot) {
                  ...
                  return ListView.builder(
                    ...
                    itemBuilder: (_, i) {
                      ...
                      final title = data['title'] as String? ?? 'Disease Simulation';
...
class DiseaseSimulationPlayScreen extends StatefulWidget {
  final Map<String, dynamic> payload;
  final String classId;

  const DiseaseSimulationPlayScreen({
    super.key,
  ...
  @override
  State<DiseaseSimulationPlayScreen> createState() => _DiseaseSimulationPlayScreenState();
}

class _DiseaseSimulationPlayScreenState extends State<DiseaseSimulationPlayScreen> {
  int _step = 0;
  int? _sel;
  bool _show = false;
...
class DiseaseSimulationBuilderDialog extends StatefulWidget {
  final String classId;
  final String contentType;

  const DiseaseSimulationBuilderDialog({
    super.key,
    required this.classId,
  ...
  @override
  State<DiseaseSimulationBuilderDialog> createState() => _DiseaseSimulationBuilderDialogState();
}

class _DiseaseSimulationBuilderDialogState extends State<DiseaseSimulationBuilderDialog> {
  ...
  Future<void> _saveSimulation() async {
    ...
    await FirestoreHelper.addContentToClass(
      ...
      {
        'type': 'simulation',
        'title': _titleCtrl.text.trim().isEmpty ? 'Disease Simulation' : _titleCtrl.text.trim(),
  ...
  Widget build(BuildContext context) => AlertDialog(
        title: const Text('Create Disease Simulation'),
        content: SizedBox(
