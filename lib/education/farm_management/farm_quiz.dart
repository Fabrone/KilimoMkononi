...
// ─── Grade helpers (mirrors other modules) ────────────────────────────────
...
// ═══════════════════════════════════════════════════════════════════════════
//  FarmQuizScreen
//  Hub that shows teacher quiz builder OR student quiz list.
//  Lives inside LearningTab's Column — no AppBar of its own.
// ═══════════════════════════════════════════════════════════════════════════
...
class _FarmQuizScreenState extends State<FarmQuizScreen> {
  final String _contentType = 'farm_management_content';

  // ── Teacher: open AI-powered quiz builder ─────────────────────────
  ...
  void _showQuizBuilder() => showDialog(
        ...
        builder: (_) => EduQuizBuilder(
          ...
          geminiSubject: 'farm financial management, budgeting, record keeping and agricultural economics in Kenya',
  ...
  Future<void> _saveQuiz(Map<String, dynamic> data) async {
    await FirestoreHelper.addContentToClass(
      ...
      {
        'type':      'quiz',
        'title':     data['title'] as String? ?? 'Farm Management Quiz',
  ...
  // ── Student: launch a specific quiz ──────────────────────────────
        ...
        Navigator.push(
          ...
          MaterialPageRoute(
            builder: (_) => EduQuizScreen(
              ...
              classId:       widget.classId,
              isPrimary:     _isPrimaryClass(widget.classId),
              geminiSubject: 'farm financial management, budgeting and agricultural economics in Kenya',
            ),
          ),
        );
      }
                ...
                builder: (context, snapshot) {
                  ...
                  return ListView.builder(
                    ...
                    itemBuilder: (_, i) {
                      ...
                      final data      = doc.data() as Map<String, dynamic>;
                      final title     = data['title'] as String? ?? 'Farm Management Quiz';
  ...
  Widget build(BuildContext context) {
    ...
    // ── Content (same for inline and routed usage) ───────────────────────
          ...
          child: ConstrainedBox(
            ...
            child: IntrinsicHeight(
              child: Column(
                ...
                children: [
                  Icon(Icons.quiz,
                      size: 90, color: primaryGreen.withValues(alpha: 0.8)),
                  const SizedBox(height: 24),
                  Text(
                    isTeacher
                        ? 'Farm Management Quizzes'
                        : 'Test your farm management knowledge',
                  ...
                  if (isTeacher) ...[
                    ...
                    OutlinedButton.icon(
                      ...
                      label: const Text('View existing quizzes'),
                      style: OutlinedButton.styleFrom(
    ...
    // ── When pushed as a route (canPop == true) wrap in Scaffold + AppBar
    //    so the user always has a back arrow.
    //    When used inline (legacy embed) return the plain content.
    if (!Navigator.of(context).canPop()) return content;
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          isTeacher ? 'Manage Quizzes' : 'Farm Quizzes',
          style: const TextStyle(
