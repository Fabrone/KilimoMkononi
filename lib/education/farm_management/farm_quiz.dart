...
class _FarmQuizScreenState extends State<FarmQuizScreen> {
  ...
  void _showQuizBuilder() {
    showDialog(
      context: context,
      builder: (_) => FarmQuizBuilderDialog(
        classId: widget.classId,
                ...
                builder: (context, snapshot) {
                  ...
                  return ListView.builder(
                    ...
                    itemBuilder: (_, i) {
                      ...
                      final title = data['title'] as String? ?? 'Farm Quiz';
              ...
              child: ConstrainedBox(
                ...
                child: IntrinsicHeight(
                  child: Column(
                    ...
                    children: [
                      Icon(Icons.quiz, size: 100, color: primaryGreen.withOpacity(0.8)),
...
class FarmQuizPlayScreen extends StatefulWidget {
  final Map<String, dynamic> payload;
  final String classId;

  const FarmQuizPlayScreen({super.key, required this.payload, required this.classId});

  @override
  State<FarmQuizPlayScreen> createState() => _FarmQuizPlayScreenState();
}

class _FarmQuizPlayScreenState extends State<FarmQuizPlayScreen> {
  int _idx = 0;
  int _score = 0;
...
class FarmQuizBuilderDialog extends StatefulWidget {
  final String classId;
  final String contentType;

  const FarmQuizBuilderDialog({super.key, required this.classId, required this.contentType});

  @override
  State<FarmQuizBuilderDialog> createState() => _FarmQuizBuilderDialogState();
}

class _FarmQuizBuilderDialogState extends State<FarmQuizBuilderDialog> {
  ...
  Future<void> _saveQuiz() async {
    ...
    await FirestoreHelper.addContentToClass(
      ...
      {
        'type': 'quiz',
        'title': _titleCtrl.text.trim().isEmpty ? 'Farm Quiz' : _titleCtrl.text.trim(),
  ...
  Widget build(BuildContext context) => AlertDialog(
        title: const Text('Create Farm Quiz'),
        content: SizedBox(
