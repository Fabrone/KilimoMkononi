...
class _FarmSimulationScreenState extends State<FarmSimulationScreen> {
    ...
    try {
      ...
      if (mounted) {
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (_) => FarmSimulationPlayScreen(
              payload: fullPayload,
  ...
  void _showSimBuilder() {
    showDialog(
      context: context,
      builder: (_) => FarmSimulationBuilderDialog(
        classId: widget.classId,
                ...
                builder: (context, snapshot) {
                  ...
                  return ListView.builder(
                    ...
                    itemBuilder: (_, i) {
                      ...
                      final title = data['title'] as String? ?? 'Farm Simulation';
...
Widget build(BuildContext context) {
  ...
  return Scaffold(
    appBar: AppBar(
      title: const Text('Farm Simulation'),
      backgroundColor: primaryGreen,
      foregroundColor: Colors.white,
    ),
    body: SafeArea(
...
class FarmSimulationPlayScreen extends StatefulWidget {
  final Map<String, dynamic> payload;
  final String classId;

  const FarmSimulationPlayScreen({super.key, required this.payload, required this.classId});

  @override
  State<FarmSimulationPlayScreen> createState() => _FarmSimulationPlayScreenState();
}

class _FarmSimulationPlayScreenState extends State<FarmSimulationPlayScreen> {
  int _step = 0;
  int? _sel;
  bool _show = false;
...
class FarmSimulationBuilderDialog extends StatefulWidget {
  final String classId;
  final String contentType;

  const FarmSimulationBuilderDialog({super.key, required this.classId, required this.contentType});

  @override
  State<FarmSimulationBuilderDialog> createState() => _FarmSimulationBuilderDialogState();
}

class _FarmSimulationBuilderDialogState extends State<FarmSimulationBuilderDialog> {
  ...
  Future<void> _saveSimulation() async {
    ...
    await FirestoreHelper.addContentToClass(
      ...
      {
        'type': 'simulation',
        'title': _titleCtrl.text.trim().isEmpty ? 'Farm Simulation' : _titleCtrl.text.trim(),
  ...
  Widget build(BuildContext context) => AlertDialog(
        title: const Text('Create Farm Simulation'),
        content: SizedBox(
