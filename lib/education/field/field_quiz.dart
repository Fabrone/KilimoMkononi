...
// ═══════════════════════════════════════════════════════════════════════════
//  FieldQuizScreen
//  Hub — no AppBar (parent field_home provides it).
// ═══════════════════════════════════════════════════════════════════════════
...
class _FieldQuizScreenState extends State<FieldQuizScreen> {
  ...
  void _showQuizBuilder() => showDialog(
        ...
        builder: (_) => EduQuizBuilder(
          ...
          topicLabel:    'Field Operations',
          geminiSubject: 'field operations, soil preparation, planting, irrigation and crop management in Kenya',
  ...
  Future<void> _saveQuiz(Map<String, dynamic> data) async {
    await FirestoreHelper.addContentToClass(
      ...
      {
        'type':      'quiz',
        'title':     data['title'] as String? ?? 'Field Quiz',
        ...
        Navigator.push(
          ...
          MaterialPageRoute(
            builder: (_) => EduQuizScreen(
              ...
              classId:       widget.classId,
              isPrimary:     _isPrimaryClass(widget.classId),
              geminiSubject: 'field operations, soil preparation, planting and crop management in Kenya',
            ),
          ),
        );
      }
              ...
              child: StreamBuilder<QuerySnapshot>(
                ...
                builder: (context, snapshot) {
                  ...
                  return ListView.builder(
                    ...
                    itemBuilder: (_, i) {
                      final doc       = filteredDocs.toList()[i];
                      final data      = doc.data() as Map<String, dynamic>;
